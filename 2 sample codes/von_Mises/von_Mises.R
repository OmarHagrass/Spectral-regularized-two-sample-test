...
dim_array <-  c(10,20,50,100)

kindx_array <- c(8,6,4,2,1)

for(d in dim_array){
  data_x <- read_csv(paste0("/storage/work/oih3/2-samples_codes/directional_von_mises/data/samples_x_dim",d,".csv"))[-1]
  for(k_indx in kindx_array){
    
      data_y <- read_csv(paste0("/storage/work/oih3/2-samples_codes/directional_von_mises/data/samples_x_kindx",k_indx,"_dim",d,".csv"))[-1]
      ...
      print(paste0("von_mises_s_",s,"_d_",d,"_n_",n,"k_indx",k_indx))
...
saveRDS(results,paste0("Directional_von_mises_results_s_",s,"_method_",method,"_kerneltyp_",kernel_type,".rds"))
